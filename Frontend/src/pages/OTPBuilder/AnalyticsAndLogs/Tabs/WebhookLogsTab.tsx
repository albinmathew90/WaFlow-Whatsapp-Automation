...
export default function WebhookLogsTab({ appId }: WebhookLogsTabProps) {
  ...
  const fetchLogs = async () => {
    ...
    try {
      ...
      const res = await fetch(`/openwa-api/otp-management/logs/${appId}/webhooks?limit=${limit}&offset=${offset}`, {
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">Webhook Deliveries</h3>
        <button 
          onClick={fetchLogs}
        ...
        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            {isLoading ? (
              ...
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-sm text-gray-500">No webhook deliveries found.</td>
              </tr>
            ) : (
              logs.map(log => (
                <tr key={log.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 truncate max-w-[200px]" title={log.destinationUrl}>
                    {log.destinationUrl}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    {getStatusBadge(log.status)}
                    {log.httpStatus && <span className="ml-2 text-xs text-gray-400">({log.httpStatus})</span>}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                    {log.retryCount}
                  </td>
