...
const CustomDatePicker = ({ value, onChange }: { value?: string, onChange?: (val: string) => void }) => {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    days.push(
      <div 
        ...
        className={`text-center text-sm cursor-pointer rounded-full flex items-center justify-center w-8 h-8 mx-auto transition-colors ${isSelected ? 'bg-admin-primary text-white shadow-md' : isToday ? 'bg-blue-50 text-admin-primary font-bold' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 font-medium'}`}
      >
        {i}
      </div>
    );
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 z-50 p-4 bg-white dark:bg-gray-900 rounded-xl shadow-xl border border-gray-100 dark:border-gray-800 w-[310px]">
          <div className="flex justify-between items-center mb-4">
            <h4 
              className="text-gray-800 dark:text-gray-200 font-bold text-sm flex items-center cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 px-2 py-1 rounded-md transition-colors"
            ...
            >
              ...
              <svg className={`w-4 h-4 ml-1 text-gray-500 dark:text-gray-400 transition-transform ${viewMode === 'years' ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
            </h4>
            
            {viewMode === 'days' && (
              <div className="flex gap-1">
                <button type="button" onClick={handlePrevMonth} className="text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-800 rounded-md transition-colors p-1"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg></button>
                <button type="button" onClick={handleNextMonth} className="text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-800 rounded-md transition-colors p-1"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg></button>
              </div>
            )}
          </div>
            ...
            <div className="grid grid-cols-4 gap-2 max-h-[220px] overflow-y-auto pr-1">
              {years.map(year => (
                <div 
                  ...
                  className={`text-center py-2 text-sm cursor-pointer rounded-lg font-medium transition-colors ${year === currentDate.getFullYear() ? 'bg-admin-primary text-white' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700'}`}
                >
                  {year}
                </div>
              ))}
        ...
        <div className="flex items-center justify-between border-b border-gray-200 dark:border-gray-700 px-3 py-1.5 bg-gray-50 dark:bg-gray-800/50 min-h-[44px]">
          ...
          {hasSelection ? (
            <div className="flex items-center flex-1 gap-3 animate-in fade-in slide-in-from-left-2 duration-200">
              <span className="text-xs font-medium text-gray-700 dark:text-gray-300">{selectedBlogs.length} selected</span>
          ...
          <div className="bg-gray-50 dark:bg-gray-800/80 border-b border-gray-200 dark:border-gray-700 px-3 py-3 flex flex-col items-start gap-3 animate-in slide-in-from-top-1 fade-in duration-150 w-full">
            {filters.length === 0 ? (
              ...
              <div className="flex flex-col w-full gap-2">
                <span className="text-[11px] text-gray-700 dark:text-gray-300 font-semibold mb-0.5">Filter Blogs where</span>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-gray-800 bg-white dark:bg-gray-900">
              {filteredBlogs.length > 0 ? (
                filteredBlogs.map(blog => (
                  <tr key={blog.id} className={`hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors group ${selectedBlogs.includes(blog.id) ? 'bg-blue-50/30' : ''}`}>
                    ...
                    {visibleColumns.date && (
                      <td className="px-3 py-2 text-xs text-black dark:text-white whitespace-nowrap">
                        {blog.date}
                      </td>
                    )}
                    ...
                    {visibleColumns.description && (
                      <td className="px-3 py-2 text-[11px] text-gray-500 dark:text-gray-400 truncate max-w-[150px]">
                        {blog.description}
                      </td>
                    )}
                ...
                <tr>
                  <td colSpan={13} className="px-3 py-8 text-center">
                    <p className="text-xs text-gray-500 dark:text-gray-400 font-medium">No blogs found matching your criteria.</p>
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center p-4 sm:p-6 bg-gray-900/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white dark:bg-gray-900 shadow-2xl w-full max-w-5xl flex flex-col my-8 rounded-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="px-8 py-5 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-white dark:bg-gray-900 sticky top-0 z-10">
              <h3 className="font-bold text-gray-900 dark:text-white text-lg tracking-tight">{editingBlogId ? 'Edit Blog' : 'Creating new Blog'}</h3>
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex">
                    ...
                    <button type="button" onClick={() => setIsCreateTopicModalOpen(true)} className="px-4 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center justify-center rounded-r-sm text-gray-500 dark:text-gray-400 transition-colors">
                ...
                <div className="flex flex-col">
                  ...
                  {coverImage ? (
                    <div className="relative group rounded-sm overflow-hidden border border-gray-200 dark:border-gray-700 w-full h-40">
                    ...
                    <div className="border border-dashed border-gray-300 dark:border-gray-600 rounded-sm p-3 bg-gray-50 dark:bg-gray-800/30 flex items-center justify-between transition-colors">
                      <button 
                        type="button" 
                        onClick={() => setIsMediaPickerOpen(true)}
                        className="px-4 py-1.5 bg-gray-900 dark:bg-gray-100 text-white dark:text-black text-[11px] font-medium rounded-sm shadow-sm hover:bg-black transition-colors"
                      >
                        Upload Media
                      </button>
        ...
        <div className="fixed inset-0 z-[60] flex items-start justify-center p-4 sm:p-6 bg-gray-900/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white dark:bg-gray-900 shadow-2xl w-full max-w-5xl flex flex-col my-8 rounded-md overflow-hidden animate-in zoom-in-95 duration-200">
            ...
            <div className="px-8 py-5 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-white dark:bg-gray-900">
              <h3 className="font-bold text-gray-900 dark:text-white text-lg tracking-tight">Creating new Blog Topic</h3>
              ...
              >
                ...
                <div className="flex flex-col">
                  ...
                  <input 
                    ...
                    className="w-full px-3 py-2 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 text-sm focus:outline-none focus:border-gray-300 rounded-sm transition-colors" 
                    required 
                  />
                </div>
