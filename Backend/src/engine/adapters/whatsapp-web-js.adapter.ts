...
export class WhatsAppWebJsAdapter extends EventEmitter implements IWhatsAppEngine {
  ...
  async initialize(callbacks: EngineEventCallbacks): Promise<void> {
    ...
    try {
      ...
      // Override logout to swallow ENOTEMPTY/EBUSY errors that cause UnhandledRejection
      const originalLogout = authStrategy.logout;
      authStrategy.logout = async (...args: any[]) => {
        try {
          await originalLogout.apply(authStrategy, args);
        } catch (e: any) {
          this.logger.warn(`LocalAuth logout cleanup failed, ignoring: ${e.message || e}`);
        }
      };
  ...
  private toMessageResult(msg: Message | undefined): MessageResult {
    if (!msg) {
      this.logger.warn('the engine returned no message for this send (common for LID destinations); returning fallback result');
      return { id: '', timestamp: Math.floor(Date.now() / 1000) };
    }
    const id = msg.id as unknown as SerializedWid | undefined;
    return { id: id?._serialized ?? id?.$1 ?? '', timestamp: msg.timestamp || Math.floor(Date.now() / 1000) };
  }
