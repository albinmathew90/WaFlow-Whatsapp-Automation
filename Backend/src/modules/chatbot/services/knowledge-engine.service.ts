...
export class KnowledgeEngineService {
  // ─── In-memory session store (per visitorSessionId) ──────────────────────────────────
  ...
  private getSession(sessionId: string): SessionContext {
    ...
    const now = Date.now();
    for (const [sid, ts] of this.sessionTimestamps.entries()) {
      if (now - ts > this.SESSION_TTL_MS) {
  ...
  private saveSession(sessionId: string, ctx: Partial<SessionContext>, userMessage: string): void {
  ...
  private scoreItem(queryTokens: string[], item: ChatbotKnowledge): number {
    ...
    for (const token of uniqueQuery) {
      ...
      if (titleTokens.includes(token)) tokenScore += 0.5;
      if (keywordTokens.includes(token)) tokenScore += 0.4;
      if (keywordTokens.some((kw) => kw.includes(token) || token.includes(kw))) tokenScore += 0.25;
      if (synonymTokens.includes(token)) tokenScore += 0.3;
  ...
  private shapeResponse(content: string, maxSentences = 3): string {
  ...
  private buildSuggestions(
    ...
    const intentSuggestions: Partial<Record<Intent, string[]>> = {
      ...
      unknown: ['How does it work?', 'Pricing & Plans', 'Contact support'],
      commission: ['Tell me about subscription', 'How does pricing work?'],
      pricing: ['Subscription plans', 'Free trial?'],
      support: ['Contact us'],
    };
  ...
  public async query(
    ...
    const ctx = this.getSession(visitorSessionId);
    const intent = this.detectIntent(message);

    const developerQueryPattern = /\b(who\s*(developed|created|built|made|owns)\s*(this|convoreach|waflow|you|the\s*bot|this\s*app|this\s*platform)|who\s*is\s*the\s*(developer|creator|author))\b/i;
    if (developerQueryPattern.test(message)) {
      this.saveSession(visitorSessionId, { lastIntent: 'company_info' }, message);
      return {
        reply: "ConvoReach is an open-source WhatsApp API Gateway.",
        intent: 'company_info',
        confidence: 1.0,
        knowledgeId: 'identity',
        knowledgeTitle: 'Developed by ConvoReach',
        suggestions: ['What features are supported?', 'API Documentation', 'About Us'],
      };
    }

    if (intent === 'greeting') {
      const reply = "Hello! 👋 Welcome! I'm here to help. You can ask me about our products, services, pricing, or anything else. What would you like to know?";
      this.saveSession(visitorSessionId, { lastIntent: intent }, message);
      return {
        reply,
        intent,
        confidence: 1.0,
        knowledgeId: null,
        knowledgeTitle: null,
        suggestions: this.buildSuggestions(intent, null, knowledgeItems),
      };
    }
    ...
    if (intent === 'thanks') {
      ...
      return {
        ...
        knowledgeId: null,
        knowledgeTitle: null,
        suggestions: this.buildSuggestions('unknown', null, knowledgeItems),
      };
    }
    ...
    let tokens = this.normalize(message);
    tokens = this.expandWithContext(tokens, ctx);

    const scored = activeItems
      .map((item) => ({ item, score: this.scoreItem(tokens, item) }))
    ...
    if (confidence >= 0.70) {
      reply = this.shapeResponse(best.item.content, 3);
      ...
      reply = `I don't have specific information about that right now. I can help you with our products, pricing, services, or other available topics. What would you like to know?`;
      this.saveSession(visitorSessionId, { lastIntent: intent }, message);
    }
    ...
    return {
      ...
      knowledgeId,
      knowledgeTitle,
      suggestions: this.buildSuggestions(intent, matchedCategory, activeItems),
    };
  }
}
