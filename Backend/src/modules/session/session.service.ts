...
export class SessionService implements OnModuleDestroy, OnModuleInit, OnApplicationBootstrap {
  ...
  async create(dto: CreateSessionDto, userId?: string): Promise<Session> {
    ...
    const existing = await this.sessionRepository.findOne({
      where: { name: dto.name, userId: userId ? userId : IsNull() },
    });
  ...
  async findAll(allowedSessions?: string[] | null, opts: ListOptions = {}, userId?: string): Promise<Session[]> {
    ...
    if (userId) {
      // CRM user-scoped: only return sessions owned by this user
  ...
  private async initializeEngine(id: string, session: Session): Promise<void> {
    ...
    const initPromise = engine.initialize({
      ...
      onMessage: (message): void => {
        ...
        (async () => {
          // Inline @lid -> phone resolution (#263), opt-in via RESOLVE_LID_TO_PHONE.
          // Done before hook execution so plugins and inbox can receive the resolved phone number.
          if (resolveFeatureFlags(this.configService).resolveLidToPhone && messageData.isLidSender && !messageData.fromMe) {
            messageData.senderPhone = await this.resolveSenderPhone(id, messageData.author ?? messageData.from);
          }

          // Execute hook for message received - plugins can modify or stop processing
          const { continue: shouldContinue, data: finalMessage } = await this.hookManager.execute('message:received', messageData, {
            sessionId: id,
            source: 'Engine',
          });
          ...
          const incoming: IncomingMessage = finalMessage;
            ...
            const chatName = incoming.contact?.name ?? incoming.contact?.pushName ?? incoming.contact?.shortName ?? incoming.contact?.verifiedName ?? undefined;
