...
export class MessageController {
  ...
  @Post('test-native-buttons')
  async testNativeButtons(@Param('sessionId') sessionId: string, @Body() body: any) {
    const engine: any = (this.messageService as any).getEngine(sessionId);
    if (!engine || !engine.client) {
      return { error: 'No active whatsapp-web.js client found for session' };
    }
    const Buttons = require('whatsapp-web.js').Buttons;
    const btnObj = new Buttons(body.text || 'Test Body', body.buttons || [{ body: 'Btn 1' }, { body: 'Btn 2' }], body.title || 'Title', body.footer || 'Footer');
    const res = await engine.client.sendMessage(body.chatId || '919119205037@c.us', btnObj);
    return { success: true, res: { id: res?.id?._serialized, timestamp: res?.timestamp } };
  }
}
