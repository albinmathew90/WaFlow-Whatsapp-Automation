...
import { CrmMediaService } from '../crm/services/crm-media.service';
...
export class MessageService {
  ...
  constructor(
    ...
    @Optional()
    private readonly crmMediaService?: CrmMediaService,
  ) {}
  ...
  async sendText(sessionId: string, dto: SendTextMessageDto): Promise<MessageResponseDto> {
    ...
    try {
      const options = finalDto.quotedMessageId ? { quotedMessageId: finalDto.quotedMessageId } : undefined;
      // Keep the 2-arg call shape for plain sends; only pass mentions when the caller supplied any.
      result = finalDto.mentions?.length
        ? await engine.sendTextMessage(finalDto.chatId, finalDto.text, finalDto.mentions, options)
        : await engine.sendTextMessage(finalDto.chatId, finalDto.text, undefined, options);
    } catch (error) {
  ...
  async sendTemplate(sessionId: string, dto: SendTemplateMessageDto): Promise<MessageResponseDto> {
    ...
    // ── Handle Location Templates (Single Bubble) ──
    // Standard WhatsApp clients cannot send a native Location attachment AND interactive buttons in the same bubble.
    // To satisfy the "together only" requirement, we embed the location as a Google Maps link at the top of the text.
    let locationHeader = '';
    if (template.type === 'Location' && content.locationLat && content.locationLong) {
      const lat = parseFloat(content.locationLat);
      const lng = parseFloat(content.locationLong);
      if (!isNaN(lat) && !isNaN(lng)) {
        const name = content.locationName ? `📍 *${content.locationName}*\n` : '';
        const addr = content.locationAddress ? `${content.locationAddress}\n` : '';
        const mapUrl = `🗺️ https://maps.google.com/maps?q=${lat},${lng}`;
        locationHeader = `${name}${addr}${mapUrl}`;
      }
    }

    const segments = [locationHeader, template.header, template.body, template.footer]
      .filter((segment): segment is string => segment != null && segment.trim().length > 0)
      .map(segment => renderTemplate(segment, vars));
    let text = segments.join('\n\n');

    const buttons = content.buttons || (template as any).buttons;
    if (Array.isArray(buttons) && buttons.length > 0) {
      ...
      for (const btn of buttons) {
        ...
        if (btn.type === 'quick_reply' || !btnVal || btnVal === btnText) {
          if (btnText) quickReplies.push(btn);
        } else {
    ...
    const mediaUrl = content.mediaUrl || (template as any).mediaUrl;
    if (mediaUrl && typeof mediaUrl === 'string' && this.crmMediaService) {
      try {
        ...
        if (filename) {
          ...
          if (buffer && buffer.length > 0) {
            ...
            if (mimetype.startsWith('video/')) {
              return await this.sendVideo(sessionId, { chatId: dto.chatId, base64, mimetype, caption: text, filename });
            } else if (mimetype.startsWith('image/')) {
              return await this.sendImage(sessionId, { chatId: dto.chatId, base64, mimetype, caption: text, filename });
            } else {
              return await this.sendDocument(sessionId, { chatId: dto.chatId, base64, mimetype, caption: text, filename });
            }
          }
        }
      } catch (err) {
        this.logger.warn(`Failed to attach media from ${mediaUrl} for template send, falling back to text`, err);
      }
    }

    return this.sendText(sessionId, { chatId: dto.chatId, text, quotedMessageId: (dto as any).quotedMessageId });
  }
  ...
  async sendImage(sessionId: string, dto: SendMediaMessageDto): Promise<MessageResponseDto> {
    ...
    try {
      const options = finalDto.quotedMessageId ? { quotedMessageId: finalDto.quotedMessageId } : undefined;
      result = await engine.sendImageMessage(finalDto.chatId, media, options);
    } catch (error) {
  ...
  async sendVideo(sessionId: string, dto: SendMediaMessageDto): Promise<MessageResponseDto> {
    ...
    try {
      const options = finalDto.quotedMessageId ? { quotedMessageId: finalDto.quotedMessageId } : undefined;
      result = await engine.sendVideoMessage(finalDto.chatId, media, options);
    } catch (error) {
  ...
  async sendAudio(sessionId: string, dto: SendAudioMessageDto): Promise<MessageResponseDto> {
    ...
    try {
      const options = finalDto.quotedMessageId ? { quotedMessageId: finalDto.quotedMessageId } : undefined;
      result = await engine.sendAudioMessage(finalDto.chatId, media, options);
    } catch (error) {
  ...
  async sendDocument(sessionId: string, dto: SendMediaMessageDto): Promise<MessageResponseDto> {
    ...
    try {
      const options = finalDto.quotedMessageId ? { quotedMessageId: finalDto.quotedMessageId } : undefined;
      result = await engine.sendDocumentMessage(finalDto.chatId, media, options);
    } catch (error) {
